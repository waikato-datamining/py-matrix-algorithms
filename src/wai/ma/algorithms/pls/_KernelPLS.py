...
class KernelPLS(AbstractMultiResponsePLS):
    ...
    def do_perform_initialization(self, predictors: Matrix, response: Matrix) -> Optional[str]:
        ...
        for current_component in range(num_components):
            ...
            while iteration_change > self.tol and iterations < self.max_iter:
                # 1)
                t: Matrix = self.K_deflated.matrix_multiply(u).normalized()
                w: Matrix = t.copy()
            ...
            self.K_deflated = part.matrix_multiply(self.K_deflated).matrix_multiply(part)
            Y = Y.sub(t.matrix_multiply(q.transpose()))
            p: Matrix = self.K_deflated.transpose().matrix_multiply(w).div(w.transpose().matrix_multiply(w).as_real())
        ...
        tT_times_K_times_U = self.T.transpose().matrix_multiply(self.K_orig).matrix_multiply(self.U)
        inv: Matrix = tT_times_K_times_U.inverse()
        self.B_RHS = inv.matrix_multiply(self.Q.transpose())
        return None
    ...
    def centralize_train_in_kernel_space(self, K: Matrix) -> Matrix:
        ...
        part: Matrix = I.sub(one.matrix_multiply(one.transpose()).div(n))
    ...
    def centralize_test_in_kernel_space(self, K: Matrix) -> Matrix:
        ...
        return (K.sub(ones_train_test_scaled.matrix_multiply(self.K_orig))).matrix_multiply(I.sub(ones_train_scaled))

    def do_perform_predictions(self, predictors: Matrix) -> Matrix:
        K_t: Matrix = self.do_transform(predictors)
        Y_hat: Matrix = K_t.matrix_multiply(self.B_RHS)
    ...
    def do_transform(self, predictors: Matrix) -> Matrix:
        ...
        return K_t.matrix_multiply(self.U)
