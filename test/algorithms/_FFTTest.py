...
from wai.ma.algorithms import FFT, OutputMode
from wai.ma.core.matrix import Matrix

from ._MatrixAlgorithmTest import MatrixAlgorithmTest


class FFTTest(MatrixAlgorithmTest):
    @classmethod
    ...
    @RegressionTest
    def orthonormal(self, subject: FFT, *resources: Matrix):
        subject.orthonormalisation = True
        return self.standard_regression(subject, *resources)

    @RegressionTest
    def real_imag_pairs(self, subject: FFT, *resources: Matrix):
        subject.output_mode = OutputMode.REAL_AND_IMAG_PAIRS
        return self.standard_regression(subject, *resources)

    @Test
    def inverses(self, subject: FFT, *resources: Matrix):
        bolts, bolts_response = resources

        # FFT and inverse
        fft: np.ndarray = subject.fft(bolts._data)
        ifft: np.ndarray = subject.ifft(fft)

        # Check if ifft == input.data
        is_equal: bool = np.allclose(ifft, bolts._data)
